...
The `release-tag.yml` workflow handles this automatically:
1. Triggered by the mn2pdf Java repository on new releases via `repository_dispatch`
2. Validates the JAR exists for the given version
3. For patch releases (no new JAR), only updates `VERSION`, keeps `MN2PDF_JAR_VERSION` at the base version
